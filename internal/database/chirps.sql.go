...
func (q *Queries) SelectAllChirps(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, selectAllChirps)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const selectAllChirpsFromAuthor = `-- name: SelectAllChirpsFromAuthor :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE user_id = $1
ORDER BY created_at
`

func (q *Queries) SelectAllChirpsFromAuthor(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, selectAllChirpsFromAuthor, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
